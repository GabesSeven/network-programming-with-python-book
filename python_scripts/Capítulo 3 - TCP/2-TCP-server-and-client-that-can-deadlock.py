...
# Gabriel Ferreira
# 2 - servidor e cliente TCP que podem entrar em deadlock

# client e servidor TCP que deixam dados demais esperando
...
def server(host, port, bytecount):
    ...
    while True:
        ...
        print('Processing up to 1024 bytes at time from', sockname)
        n = 0
...
def client(host, port, bytecount):
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    # arredonda para um múltiplo de 16
    bytecount = (bytecount + 15) // 16 * 16
    # mensagem de 16 bytes a ser repetida várias vezes
